...
public class CommentsDAO {
	...
	public int delete(int cnum, Connection con) throws Exception{
		int result= 0;
		String sql = "delete shop_qna_comments where cnum=?";
		PreparedStatement st = con.prepareStatement(sql);
		st.setInt(1, cnum);
	...
	public int update(CommentsDTO commentsDTO, Connection con) throws Exception{
		int result =0;
		String sql = "update shop_qna_comments set contents=? where cnum=?";
		PreparedStatement st = con.prepareStatement(sql);
	...
	public List<CommentsDTO> selectList(SearchRow searchRow, int num, Connection con)throws Exception{
		...
		String sql = "select * from "
				...
				+ "(select * from shop_qna_comments where num =? order by cnum desc) C) "
				+ "where R between ? and ?";
		PreparedStatement st = con.prepareStatement(sql);
		st.setInt(1, num);
