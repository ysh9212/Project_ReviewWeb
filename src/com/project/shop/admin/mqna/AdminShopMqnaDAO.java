...
public class AdminShopMqnaDAO implements BoardDAO{
	...
	public BoardDTO selectOne(int no, Connection con) throws Exception {
		...
		if(rs.next()) {
			adminShopMqnaDTO.setNo(rs.getInt("no"));
			adminShopMqnaDTO.setTitle(rs.getString("title"));
			adminShopMqnaDTO.setContents(rs.getString("contents"));
			adminShopMqnaDTO.setWriter(rs.getString("writer"));
			adminShopMqnaDTO.setReg_date(rs.getString("reg_date"));
			adminShopMqnaDTO.setHit(rs.getInt("hit"));
			adminShopMqnaDTO.setRecommend(rs.getInt("recommend"));
			adminShopMqnaDTO.setDecommend(rs.getInt("decommend"));
		}
		rs.close();
		st.close();
		
		return adminShopMqnaDTO;
	...
	public int insert(BoardDTO boardDTO, Connection con) throws Exception {
		int result = 0;
		String sql = "insert into shop_mqna values(shop_mqna_seq.nextval,?,?,?,sysdate,0,0,0)";
		PreparedStatement st = con.prepareStatement(sql);
	...
	public int update(BoardDTO boardDTO, Connection con) throws Exception {
		int result = 0;
		String sql = "update shop_mqna set title=?, contents=? where no=?";
		PreparedStatement st = con.prepareStatement(sql);
		st.setString(1, boardDTO.getTitle());
		st.setString(2, boardDTO.getContents());
	...
	public int delete(int no, Connection con) throws Exception {
		int result = 0;
		String sql = "delete shop_mqna where no=?";
		PreparedStatement st = con.prepareStatement(sql);
		st.setInt(1, no);
		result = st.executeUpdate();
		st.close();
