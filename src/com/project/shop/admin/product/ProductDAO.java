...
public class ProductDAO {
	...
	public int update(ProductDTO productDTO, Connection con) throws Exception{
		int result = 0;
		String sql = "update product set title=?, detail=?, price=?, stock=? where pno=?";
		PreparedStatement st = con.prepareStatement(sql);
		st.setString(1, productDTO.getTitle());
		st.setString(2, productDTO.getDetail());
		st.setInt(3, productDTO.getPrice());
		st.setInt(4, productDTO.getStock());
		st.setInt(5, productDTO.getPno());
